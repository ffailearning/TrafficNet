...
from ultralytics.nn.AFusion import AFusion
from ultralytics.nn.TPFusion import TPFusion
from ultralytics.nn.autobackend import check_class_names
from ultralytics.nn.MLFENet import AFE, PoolDown
...
def parse_model(d, ch, verbose=True):
    ...
    ################################################################################################
    msn = [round(x[1] * depth) for x in d['backbone'] if x[2] in ['AFE']]
    dpr = [x.item() for x in torch.linspace(0, 0.01, sum(msn))]
    msf_id = 0
    ################################################################################################
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if isinstance(c2, list):
            ...
            if m is AFE:
                m_ = nn.Sequential(*(m(*args, drop_path=dpr[msf_id + z]) for z in range(n))) if n > 1 else m(*args)
                msf_id += n
            else:
